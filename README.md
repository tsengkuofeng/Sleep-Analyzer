# 睡眠與運動時機分析器 Two-Process Model + Exercise Timing
...
> 問卷擴充為 4 大類即時問項，演算法加入主觀嗜睡度、睡眠效率的個人化修正，並新增運動時機建議引擎（紅黃綠燈）。核心兩歷程模型方程式與參數維持不變、經過自動化測試驗證（見下方「驗證方式」）。運動「類型」問項已移除——文獻顯示種類對睡眠效益的影響不大，不需要讓使用者選擇。
...
以上皆為自我觀察用的參考資訊，**不是醫療器材，也不能取代睡眠專科醫師或運動醫學專業的診斷與建議**。
...
- 依範例資料（平常06:30起床、偏好08:00起床、實際06:30起床、昨晚睡5.5h、ESS總分7/12、入睡花35分鐘、醒來2次）跑出的結果符合預期：睡眠效率88%、睡眠負債2.5h、觸發「清醒努力」風險警示、綠燈運動時段的結束時間精確落在「預測入睡時間−4小時」
